...
3. **Show it, then wait.** Build the whole file and put it up with the path it
   ...
   session writing several drafts writes each on its own, so the target is
   already there when the dependent one is checked. `superseded_by` gets no such
   check and cannot: a supersede is one approval unit, so when the predecessor
   is checked its successor is deliberately not on disk yet. What carries that
   half is the write order below, the successor first, and `--open`, which
   reports a target that is gone instead of reading the draft as unblocked. On a
